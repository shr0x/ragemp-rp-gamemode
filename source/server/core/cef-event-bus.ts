import { CefData } from "@shared/cef-data";
...
class Cef_Event {
    ...
    register(
        ...
        const eventTarget = String(page);
        const eventPointer = String(pointer);

        if (!this.eventsInMemory.some((event) => event.target === eventTarget && event.name === eventPointer)) {
            const _event = new mp.Event(`server::${eventTarget}:${eventPointer}`, handler);
            this.eventsInMemory.push({ target: eventTarget, name: eventPointer, handler, _event });
            return _event;
        } else {
            console.log("------------------------------------------------------------");
            throw new Error(`Event: "${eventTarget}", "${eventPointer}" was found duplicated`);
        }
    }
